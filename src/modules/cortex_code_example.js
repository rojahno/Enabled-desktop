...
class Cortex {
    ...
    async loadProfile(profileName) {
        this.socket.onopen = async () => {
            await this.checkGrantAccessAndQuerySessionInfo()
           // console.log("hurla");
            this.setupProfile(await this.authToken, await this.headsetId, profileName, "load").then((data) => {
                console.log('\r\n');
                console.log("Data fra loadProfile:");
                console.log(data);
                console.log('\r\n');
                let parsedData = JSON.parse(data);
                console.log(parsedData);
                return parsedData;
                //console.log(parsedData.result[1].name);
                this.subRequest(['com'], this.authToken, this.sessionId)
